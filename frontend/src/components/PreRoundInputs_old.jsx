...
export default function PreRoundInputs({ players, onSubmit, onBack }) {
  ...
  const [playerData, setPlayerData] = useState(() =>
    players.map((name, i) => ({ name, mulligan_used: false, metal_hits: 0, arrival_order: i + 1, new_players_brought: 0 }))
  )
  ...
  return (
    <div style={s.wrap}>
      <pre style={{color:'red', fontSize:10}}>{JSON.stringify(players?.slice(0,1), null, 2)}</pre>
      <div style={s.header}>
      ...
      <div style={s.actions}>
        <button style={s.submitBtn} onClick={() => onSubmit({ multiplier, players: playerData })}>→ Process Round</button>
